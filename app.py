    ...
    if request.method == 'POST':
        if request.form.get("login"):
            ...
            if username == '' or password == '':
                ...
                if records == []:
                    return render_template('error_page.html', error='There is no such user registrated!')
                else:
...
def registration():
    if request.method == 'POST':
        ...
        records = list(cursor.fetchall())
        if records != []:
            return render_template('error_page.html', error='User with such login is already registrated!')
        else:
